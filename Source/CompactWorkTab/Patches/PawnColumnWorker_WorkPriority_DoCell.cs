...
{
    public static void Postfix(PawnColumnWorker_WorkPriority __instance, Rect rect, Pawn pawn)
    {
        if (!ModSettings.UseScrollWheel)
        {
            return;
        }

        if (!Mouse.IsOver(rect) || pawn.WorkTypeIsDisabled(__instance.def.workType))
        {
            return;
        }
        ...
        var workTypeWasActive = pawn.workSettings.WorkIsActive(__instance.def.workType);
        int newPriority;
        ...
        if (Event.current.delta.y > 0)
        {
            if (Find.PlaySettings.useWorkPriorities)
            {
                ...
                if (newPriority > Cache.MaxPriority)
                {
                    newPriority = 0;
                }
            }
        ...
        if (Event.current.delta.y < 0)
        {
            ...
            {
                newPriority = pawn.workSettings.GetPriority(__instance.def.workType) - 1;
                if (newPriority < Cache.MinPriority)
                {
                    newPriority = Cache.MaxPriority;
                }
            }
            else
            {
        ...
        if (!workTypeWasActive && pawn.workSettings.WorkIsActive(__instance.def.workType) &&
            __instance.def.workType.relevantSkills.Any() &&
            pawn.skills.AverageOfRelevantSkillsFor(__instance.def.workType) <= 2f)
        {
